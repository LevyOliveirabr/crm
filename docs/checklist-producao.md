...
4b. Supabase → SQL Editor: aplicar `supabase/migrations/0008_multi_empresa.sql` **antes** de publicar o código multi-empresa (empresas vendedoras, perfil por empresa, categorias/catálogos, `emitente_id` com default para a janela migration→deploy). Depois do deploy estável, aplicar `0009_multi_empresa_limpeza.sql` (remove a tabela `emitente`, os defaults e `usuarios.gerente_id`). Ordem obrigatória: 0008 → deploy → 0009.
