...
export default async function ProdutosConfigPage({
  ...
  return (
    <Secao
      titulo="Produtos"
      meta={`${produtos.length} ${produtos.length === 1 ? "produto" : "produtos"}`}
      acoes={<BotaoExportar tela="produtos" filtros={{ q: q || null }} />}
    >
      <p className="mb-4 text-sm text-muted-foreground">
        Cadastro por empresa vendedora: código, categoria, preço base, link no
        site e catálogo.{" "}
        {escopo.emitente
          ? `Mostrando: ${escopo.emitente.nome}.`
          : "Mostrando todas as empresas."}
      </p>
